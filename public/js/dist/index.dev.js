...
var resultsContainer = document.querySelector(".results"); // --- ROBUST TOUCH HANDLING START ---
// This section implements a custom tap detection system to bypass
// potential click-blocking issues on iOS Safari/Opera.

var touchStartX = 0;
var touchStartY = 0;
var lastTouchTime = 0; // 1. Capture Touch Start
...
document.addEventListener("touchend", function (e) {
  if (e.changedTouches && e.changedTouches.length > 0) {
    ...
    var touchEndY = e.changedTouches[0].screenY; // Calculate movement distance

    var diffX = Math.abs(touchEndX - touchStartX);
    var diffY = Math.abs(touchEndY - touchStartY); // If movement is small (< 10px), consider it a tap

    if (diffX < 10 && diffY < 10) {
      // Check if we tapped a target we care about
      var target = e.target; // A. Job Card Tap
