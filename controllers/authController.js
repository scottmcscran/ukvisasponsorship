...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  const currentUser = await User.findById(decoded.id)
    .populate(`activeJobsCount`)
    .populate(`featuredJobsCount`)
    .populate(`basicJobsCount`)
    .populate(`disabledJobsCount`);

  if (!currentUser)
    return next(
      new AppError(`User associated with this login token does not exist.`, 404)
    );
...
exports.requireActiveSubscription = catchAsync(async (req, res, next) => {
  ...
  return next(
    new AppError(`Active subscription required. Please upgrade your plan.`, 403)
  );
});
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError(`Reset token expired or invalid.`, 400));
...
exports.checkAccountStatus = catchAsync(async (req, res, next) => {
  ...
  if (!req.user.companyProfile) return next();

  if (req.user.companyProfile.accStatus === "unverified") {
    return next(
      new AppError(
        "Your account is pending verification. You cannot post jobs yet.",
        403
      )
    );
  }
...
exports.checkJobPostLimits = catchAsync(async (req, res, next) => {
  // Skip check for admins
  if (req.user.role === "admin") return next();
